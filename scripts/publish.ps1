...
if ($Target.Equals("Debug")) {
    ...
    $assets = New-Item -Type Directory -Path "$DeployPath\$name\Assets" -Force
    Write-Host "Copy $TargetAssembly to $plug"

    Copy-Item -Path "$ProjectPath\Assets\axis_*.png" -Destination "$assets" -Force
...
if($Target.Equals("Release")) {
    ...
    $plug = New-Item -Type Directory -Path "$PackagePath\plugins\$name" -Force
    $assets = New-Item -Type Directory -Path "$PackagePath\plugins\$name\Assets" -Force
    Copy-Item -Path "$ProjectPath\Assets\axis_*.png" -Destination "$assets" -Force
